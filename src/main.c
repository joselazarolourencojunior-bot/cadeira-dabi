...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define OUT_SOBE_ASSENTO_PORT GPIOA
#define OUT_SOBE_ASSENTO_PIN  GPIO_PIN_0
#define OUT_DESCE_ENCOSTO_PORT GPIOA
#define OUT_DESCE_ENCOSTO_PIN  GPIO_PIN_1
#define OUT_SOBE_ENCOSTO_PORT GPIOA
#define OUT_SOBE_ENCOSTO_PIN  GPIO_PIN_2
#define OUT_DESCE_ASSENTO_PORT GPIOA
#define OUT_DESCE_ASSENTO_PIN  GPIO_PIN_3
#define OUT_REFLETOR_PORT GPIOA
#define OUT_REFLETOR_PIN  GPIO_PIN_4

#define IN_VZ_PORT GPIOA
#define IN_VZ_PIN  GPIO_PIN_5
#define IN_PT_PORT GPIOA
#define IN_PT_PIN  GPIO_PIN_6
...
static DebouncedButton_t btnVZ = { IN_VZ_PORT, IN_VZ_PIN, 0, 0, 0 };
static DebouncedButton_t btnPT = { IN_PT_PORT, IN_PT_PIN, 0, 0, 0 };
...
static void StopAllMotors(void)
{
  HAL_GPIO_WritePin(OUT_SOBE_ASSENTO_PORT,
                    OUT_SOBE_ASSENTO_PIN | OUT_DESCE_ASSENTO_PIN | OUT_SOBE_ENCOSTO_PIN | OUT_DESCE_ENCOSTO_PIN,
                    GPIO_PIN_RESET);
}
...
{
  StopAllMotors();
  HAL_GPIO_WritePin(OUT_DESCE_ASSENTO_PORT, OUT_DESCE_ASSENTO_PIN, GPIO_PIN_SET);
  HAL_GPIO_WritePin(OUT_SOBE_ENCOSTO_PORT, OUT_SOBE_ENCOSTO_PIN, GPIO_PIN_SET);
...
{
  StopAllMotors();
  HAL_GPIO_WritePin(OUT_SOBE_ASSENTO_PORT, OUT_SOBE_ASSENTO_PIN, GPIO_PIN_SET);
  HAL_GPIO_WritePin(OUT_DESCE_ENCOSTO_PORT, OUT_DESCE_ENCOSTO_PIN, GPIO_PIN_SET);
  operationStartMs = now;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    uint8_t anyButtonPressed = (btnVZ.stable || btnPT.stable) ? 1 : 0;
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA,
                    OUT_SOBE_ASSENTO_PIN | OUT_DESCE_ENCOSTO_PIN | OUT_SOBE_ENCOSTO_PIN | OUT_DESCE_ASSENTO_PIN | OUT_REFLETOR_PIN,
                    GPIO_PIN_RESET);

  GPIO_InitStruct.Pin = OUT_SOBE_ASSENTO_PIN | OUT_DESCE_ENCOSTO_PIN | OUT_SOBE_ENCOSTO_PIN | OUT_DESCE_ASSENTO_PIN | OUT_REFLETOR_PIN;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  GPIO_InitStruct.Pin = IN_VZ_PIN | IN_PT_PIN;
